...
### end here        
    ...
    ### end here         
        # 차트를 저장할 디렉토리 생성
        chart_dir = 'C:/Users/user/OneDrive - pusan.ac.kr/바탕 화면/stock_report'
    ...
    # report.add_title(f'[{company_name}] 주식 보고서',"보고서 작성일 : {date}".format(date=today))
    report.add_title(f'[{company_name}] 주식 보고서',f"보고서 작성일 : {today}","provider: RX2")
    
    # 보고서에 차트와 테이블 추가
    report.add_table_chart(f"{company_name}, {stock_data.iloc[0]['Close']}원에 거래 마감",
                           os.path.join(chart_dir, f'{company_name}_chart.png'),
                           os.path.join(chart_dir, f'{company_name}_table.png'))
...
### start here
    ...
    # # 매일 지정된 시간에 이메일 전송
    # schedule.every().day.at("21:20").do(send.send_email)
    # while True:
